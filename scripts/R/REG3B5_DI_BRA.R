...
# output paths
dir_(paste0(dir_proc,"BRA/"))
dir_out_ib <- dir_(paste0(dir_proc,"BRA/", damsdata,"/"))
dir_(paste0(dir_proc,"BRA/", damsdata,"/model_DI_occurrence/"))
dir_out <- dir_(paste0(dir_proc,"BRA/", damsdata,"/model_DI_occurrence/",mvrs_type,"/"))
dir_out_tabs <- dir_(paste0(dir_proc,"BRA/", damsdata,"/model_DI_occurrence/",mvrs_type,"/tabs/"))


#>> Hydrobasins data

# read hydrobasins data
hb_data <- foreach(i = c('sa'),.combine = 'rbind') %do% read_sf(paste0(dir_hybas12,'/hybas_lake_',i,'_lev12_v1c.shp')) # REGION SPECIFIC
# extract only region HB units
...
#>> Dams data
cat('Load dams data..\n')
dams <- readRDS(paste0('Dams_impact/proc/dams_BRA_hydrobasins_',damsdata,'.rds')) # REGION SPECIFIC
dams_cur=dams
...
saveRDS(reg_ib_tab,paste0(dir_out_ib,"BRA_interbasins_",damsdata,'.rds')) # REGION SPECIFIC
cat('DI_interbasins Done..\n')


### ----------------------- prep_DI_create_SR_PLR_tabs ------------------------------------------------------------------
cat('prep_DI_create_SR_PLR_tabs..\n')

tab_hb <- left_join(reg_ib_tab,hb_data) %>% st_as_sf()

#some basins dont have dams, they will have INTER_HYBAS_ID zero, we'd like to remove these from the selection
...
#st_write(SR_cur, paste0('Dams_impact/proc/USA/interbasins_USA_hydrobasins_',damsdata,'.rds'), append=F)
...
fish12 <- foreach(cont = c('sa'),.combine='rbind') %do% {# REGION SPECIFIC
...
build_tab <- function(i){
  ...
  # if lentic_only, dont produce results. Otherwise check if marine/brackish.diad or not
  if(lentic_only ==1){
    sp_ib$hab_nodams=NA
    sp_ib$hab_curdams=NA
  }else{
    
    # if marine/brackish/diad
...
cat('Start interb_species range parallel running \n')
# do it parrallel
...
saveRDS(SR_cur %>% as.data.frame() %>% dplyr::select(-geometry), paste0(dir_out_tabs,'RESinterbasins_cur_fwonly.rds'))
...
tab_mb$PAFrange_cur=1-(tab_mb$Rtot-tab_mb$Arange_cur)/tab_mb$Rtot # = CRL
