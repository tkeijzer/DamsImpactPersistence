...
# output paths
dir_(paste0(dir_proc,"MEK/"))
dir_out_ib <- dir_(paste0(dir_proc,"MEK/", damsdata,"/"))
dir_(paste0(dir_proc,"MEK/", damsdata,"/model_DI_occurrence/"))
dir_out <- dir_(paste0(dir_proc,"MEK/", damsdata,"/model_DI_occurrence/",mvrs_type,"/"))
dir_out_tabs <- dir_(paste0(dir_proc,"MEK/", damsdata,"/model_DI_occurrence/",mvrs_type,"/tabs/"))


#>> Hydrobasins data

# read hydrobasins data
hb_data <- foreach(i = c('as'),.combine = 'rbind') %do% read_sf(paste0(dir_hybas12,'/hybas_lake_',i,'_lev12_v1c.shp')) %>% # REGION SPECIFIC
...
#>> Dams data
cat('Load dams data..\n')
dams <- readRDS(paste0('Dams_impact/proc/dams_MEK_hydrobasins_',damsdata,'.rds')) # REGION SPECIFIC
dams_cur=dams
...
saveRDS(reg_ib_tab,paste0(dir_out_ib,"MEK_interbasins_",damsdata,'.rds')) # REGION SPECIFIC
...
fish12 <- foreach(cont = c('as'),.combine='rbind') %do% {
  tab <- read.csv(paste0(dir_sp12, '/fish_hybas_hybas12withLakes_',cont,'.csv')) # REGION SPECIFIC
  return(tab)
}
