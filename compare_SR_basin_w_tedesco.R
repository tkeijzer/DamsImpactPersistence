...
#library(dplyr);library(foreach);library(sf);library(raster);library(fasterize);library(terra)
sf::sf_use_s2(FALSE)
...
hblakes <- foreach(cont = c('af','ar','as','au','eu','gr','na','sa','si'),.combine='rbind') %do% {
  poly <- read_sf(paste0('Hybas_lake/hybas_lake_',cont,'_lev08_v1c.shp')) %>% st_point_on_surface()
  return(poly)
}
...
dev.off()
ggsave('Dams_impact/figs/compare_SR_Tedesco_IUCNdata.jpg',figure2, height = 200, width = 178,
       dpi = 1000,units = 'mm')
