...
#Bas_tab <- do.call('rbind',parallel::mclapply(1:10,create_tab,mc.silent = TRUE,mc.cores = 10))
...
# some stats
T_fb=na.omit(unique(Tsp$fb_name)) # 14,470 fishbase names (14,953 species binomials)
IUCN_fb=na.omit(unique(fb$fb_name)) # 10,888 fishbase names (11,465 species binomials)
sum(T_fb %in% IUCN_fb) #8,814 matches
sum(IUCN_fb %in% T_fb)

# so the IUCN observations of 2,074 species could not be taken into account
# these species are not in the Tedesco dataset
# on the other hand, 3,582 species from Tedesco are not in the IUCN data.

# for comparison below get things from decideMVA

#test=fb %>% filter(!(fb_name %in% T_fb)) %>% filter(!is.na(fb_name)) #2,075
#test2=fish %>% filter(binomial %in% test$binomial) %>% select(binomial,yrcompiled) %>% distinct()
#test3=fish %>% filter(binomial %in% test$binomial) %>% group_by(binomial) %>% summarise(year=min(yrcompiled))

#table(test2$yrcompiled) # Tedesco is form 2017
#table(test3$year) 
#  1,183 species that are not mentioned in tedesco have solely observations with yrcompiled after 2017.

# of the 4,762 species with observations after decideMVA selection
# 505 of the species was removed when using selection as the fish was not
# present in the Tedesco dataset (or could not be matched)

#test=fb %>% filter(binomial %in% unique(sp$binomial)) #sp from decideMVA after selection other stuff
#sum(test$fb_name %in% T_fb) 
